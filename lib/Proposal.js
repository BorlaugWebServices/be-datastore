...
module.exports = class Proposal {
    ...
    async save(proposal) {
        let id = proposal.id;
        let resultSet = await this.db("proposal").where("id", id);
        if(resultSet && resultSet.length === 0) {
            await this.db("proposal").insert(proposal);
        }
        let cache_proposal = await this.cacheGet(`proposal:${id}`);
        if(!cache_proposal) {
    ...
    async saveActivity(activity) {
        let activities = await this.db("proposal_activity").where("proposal_id", activity.proposal_id)
