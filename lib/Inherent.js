...
module.exports = class Inherent {
    ...
    async save(inherent) {
        ...
        let resultSet = await this.db("inherent").where("id", inherent.id);
        if(resultSet && resultSet.length === 0) {
            await this.db("inherent").insert(dbInherent);
        }
        let cache_inherent = await this.cacheGet(`inh:${inherent.id}`);
        if(!cache_inherent) {
            await this.cacheSet(`inh:${inherent.id}`, JSON.stringify(inherent), 'EX', inherent.significant ? this.ttlMax : this.ttlMin);
        }
    }
