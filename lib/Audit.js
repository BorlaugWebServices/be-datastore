const debug       = require("debug")("be-datastore:Audit");
...
module.exports = class Audit {
    ...
    async save(audit) {
        let id = audit.id;
        let resultSet = await this.db("audit").where("id", id);
        if(resultSet && resultSet.length === 0) {
            await this.db("audit").insert(audit);
        }
        let cache_audit = await this.cacheGet(`audit:${id}`);
        if(!cache_audit) {
    ...
    async saveActivity(activity) {
        let activities = await this.db("audit_activity")
            .where("audit_id", activity.audit_id)
    ...
    async get(auditid) {
        ...
        if(audit) {
            ...
            if(audit) {
                await this.cacheSet(`audit:${audit.id}`, JSON.stringify(audit), 'EX', this.ttlMax);
            }
        }
        return audit;
    }
    ...
    async getActivities(auditid) {
        let activities = await this.db("audit_activity").where("audit_id", auditid).select("tx_hash");
