...
const debug = Debug('be-datastore:Migration');
...
export default class Migration {
  ...
  async up() {
    return Promise.all([
      this.db.schema.createTable('block', (table) => {
        ...
        table.index(['hash'], 'block_hash_index');
      }),
      this.db.schema.createTable('transaction', (table) => {
        ...
        table.string('tip');
        table.bigInteger('timestamp').unsigned();
        table.index(['hash'], 'transaction_hash_index');
      }),
      ...
      this.db.schema.createTable('identity_activity', (table) => {
        ...
        table.index(['did'], 'identity_activity_did_index');
      }),
      ...
      this.db.schema.createTable('asset_registry_activity', (table) => {
        ...
        table.index(['registry_id'], 'asset_registry_activity_registry_id_index');
      }),
      ...
      this.db.schema.createTable('asset_activity', (table) => {
        ...
        table.index(['asset_id'], 'asset_activity_asset_id_index');
      }),
      ...
      this.db.schema.createTable('registry', (table) => {
        table.string('id').primary();
        table.string('creator');
        table.string('creator_group');
        table.integer('blockNumber').notNullable().unsigned();
        table.string('blockHash').notNullable();
        table.string('extrinsicHash').notNullable();
        table.bigInteger('timestamp').unsigned();
      }),
      this.db.schema.createTable('registry_activity', (table) => {
        table.string('registry_id').notNullable();
        table.string('tx_hash');
        table.unique(['registry_id', 'tx_hash']);
        table.index(['registry_id'], 'registry_activity_registry_id_index');
      }),
      this.db.schema.createTable('definition', (table) => {
        table.string('id').primary();
        table.string('registry_id').notNullable();
        table.string('creator');
        table.string('creator_group');
      ...
      this.db.schema.createTable('definition_activity', (table) => {
        table.string('definition_id').notNullable();
        table.string('tx_hash');
        table.unique(['definition_id', 'tx_hash']);
  ...
  async down() {
    return Promise.all([
      ...
      this.db.schema.dropTable('registry'),
      this.db.schema.dropTable('registry_activity'),
      this.db.schema.dropTable('definition'),
      this.db.schema.dropTable('definition_activity'),
