...
module.exports = class Log {
     ...
     */
    async save(log) {
        let resultSet = await this.db("log").where("id", log.id);
        if(resultSet && resultSet.length === 0) {
            await this.db("log").insert(log);
        }
        let cache_log = await this.cacheGet(`log:${log.id}`);
        if(!cache_log) {
            await this.cacheSet(`log:${log.id}`, JSON.stringify(log), 'EX', log.significant ? this.ttlMax : this.ttlMin);
        }
    }
    ...
    async getList(logids) {
        ...
        let logs = keys.length > 0 ? await this.cacheMget(keys) : [];
        ...
        if(logs.length === keys.length) {
            ...
            _.map(logs, async (log) => {
                await this.cacheSet(`log:${log.id}`, JSON.stringify(log), 'EX', this.ttlMin);
            });
        }
        return logs;
    }
