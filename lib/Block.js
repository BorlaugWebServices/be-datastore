...
module.exports = class Block {
    ...
    async save(block) {
        ...
        //TODO: why are we checking for hash? If this is needed we must add an index on the hash column.
        let blocks = await this.db("block").where("number", block.number).orWhere("hash", block.number);
        if (blocks && blocks.length === 0) {
            await this.db("block").insert(dbBlock);
        }
        if (block.number > latestBlock) {
            // debug("latestBlockNumber", block.number.toString());
            await this.cacheSet("latestBlockNumber", block.number.toString());
        }
        await this.cacheSet(`block:${block.number}`, JSON.stringify(block), 'EX', block.significant ? this.ttlMax : this.ttlMin);
        await this.cacheSet(`block:${block.hash}`, JSON.stringify(block), 'EX', block.significant ? this.ttlMax : this.ttlMin);
        if (block.number >= latestBlock) {
    ...
    async get(numberOrHash) {
        ...
        try {
            ...
            if (block) {
                ...
                //TODO: we need to index the hash column for this to be efficient.
                //TODO: we know from previous code whether we have a block or a hash. We should only query the one we have.
    ...
    async getList(numbers) {
        ...
        let blocks = keys.length > 0 ? await this.cacheMget(keys) : [];
        if (blocks.length === keys.length) {
            ...
            _.map(blocks, async (block) => {
                await this.cacheSet(`block:${block.number}`, JSON.stringify(block), 'EX', this.ttlMin);
            });
        }
        return blocks;
    }
