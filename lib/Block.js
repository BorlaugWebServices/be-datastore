...
module.exports = class Block {
    ...
    async latestBlockNumber() {
        ...
        if(!latestBlock) {
            ...
            if(latestBlock[0].max) {
                await this.cacheSet("latestBlockNumber", latestBlock[0].max);
            }
        }
    ...
    async save(block) {
        ...
        if(block.number > latestBlock) {
            await this.cacheSet("latestBlockNumber", block.number);
        }
        await this.cacheSet(`block:${block.number}`, JSON.stringify(block), 'EX', this.ttl);
        debug(block.number,latestBlock,block.number === latestBlock)
        if(block.number === latestBlock) {
